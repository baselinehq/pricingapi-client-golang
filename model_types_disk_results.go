/*
Pricing API

Provides API level access to the Costgraph Services.

API version: 1.0
Contact: contact@baselinehq.cloud
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package pricing_api_client

import (
	"encoding/json"
)

// checks if the TypesDiskResults type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TypesDiskResults{}

// TypesDiskResults struct for TypesDiskResults
type TypesDiskResults struct {
	AdditionalProperties map[string]interface{}
}

type _TypesDiskResults TypesDiskResults

// NewTypesDiskResults instantiates a new TypesDiskResults object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTypesDiskResults() *TypesDiskResults {
	this := TypesDiskResults{}
	return &this
}

// NewTypesDiskResultsWithDefaults instantiates a new TypesDiskResults object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTypesDiskResultsWithDefaults() *TypesDiskResults {
	this := TypesDiskResults{}
	return &this
}

func (o TypesDiskResults) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TypesDiskResults) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *TypesDiskResults) UnmarshalJSON(data []byte) (err error) {
	varTypesDiskResults := _TypesDiskResults{}

	err = json.Unmarshal(data, &varTypesDiskResults)

	if err != nil {
		return err
	}

	*o = TypesDiskResults(varTypesDiskResults)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableTypesDiskResults struct {
	value *TypesDiskResults
	isSet bool
}

func (v NullableTypesDiskResults) Get() *TypesDiskResults {
	return v.value
}

func (v *NullableTypesDiskResults) Set(val *TypesDiskResults) {
	v.value = val
	v.isSet = true
}

func (v NullableTypesDiskResults) IsSet() bool {
	return v.isSet
}

func (v *NullableTypesDiskResults) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTypesDiskResults(val *TypesDiskResults) *NullableTypesDiskResults {
	return &NullableTypesDiskResults{value: val, isSet: true}
}

func (v NullableTypesDiskResults) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTypesDiskResults) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


